...
fn init_tracing(verbose: Verbosity) {
    ...
    // Our crates at the chosen level; deps (wreq, hyper, …) stay at warn and chromiumoxide is
    // muted, so `-v debug` doesn't drown the log. `RUST_LOG`, if set, takes over completely.
    let default = format!("warn,mara={lvl},mara_solver={lvl},mara_core={lvl},chromiumoxide=off");
                    ...
                    async move {
                        let result = client
                            .fetch_browser(&url, |page| async move {
                                Ok(mara::settle::wait_full_load(&page, Duration::from_secs(15))
                                    .await)
                            })
                            .await;
                        FetchResult {
